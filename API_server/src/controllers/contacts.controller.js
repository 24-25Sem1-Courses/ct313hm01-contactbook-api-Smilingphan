...
async function getContactsByFilter(req, res, next) {
    ...
    try {
        ...
        console.log(error);
        return next(new ApiError(500, 'An error occerred while retrieving contacts'));
    }
    return res.json(
        JSend.success({
...
async function getContact(req, res, next) {
    ...
    try {
        ...
        console.log(error);
        return next(new ApiError(500, `Error retrieving contact with id=${id}`)); // Sửa lỗi sử dụng chuỗi template string
    }
}
...
async function deleteContact(req, res, nextt) {
    const { id } = req.params
    try {
